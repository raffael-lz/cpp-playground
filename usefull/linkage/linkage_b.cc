...
// f is declared here in global scope. In global there can be only one
// delcaration even linkage_b.cc is not included anywhere, only because of
// linking process, g() can access f()
void f()
{
    ...
    std::cout << __FILE_NAME__ << "::" << __FUNCTION__ << " exit: " << std::endl
              << std::endl;
}
